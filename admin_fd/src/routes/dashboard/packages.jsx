...
export function PackagesPage() {
  ...
  const toggleFeatured = async (pkg) => {
    ...
    try {
      ...
      setPackages((prev) =>
        prev.map((p) =>
          p.package_id === pkg.package_id ? { ...p, is_featured: !p.is_featured } : p,
        ),
      );
    } catch (err) {
      setPageError(err instanceof Error ? err.message : "Toggle failed");
              ...
              {filteredPackages.map((pkg) => (
                ...
                >
                  <td className="py-2 px-3">
                    ...
                    {pkg.package_code && (
                      <p className="mt-1 w-16 text-center text-[11px] font-mono text-muted-foreground">
                        {pkg.package_code}
                      </p>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-3 text-foreground">
                    {fmtDuration(pkg.estimated_duration)}
                  </td>
                  <td className="py-3 px-3 text-foreground">{pkg.max_capacity}</td>
                  <td className="py-3 px-3">
                    <span
                      className={`text-sm font-medium ${pkg.is_active ? "text-accent" : "text-muted-foreground"}`}
                    >
                  ...
                  <td className="py-3 px-3">
                    <div className="flex gap-1">
                      ...
                      >
                        <Star
                          className={`h-3 w-3 ${pkg.is_featured ? "fill-accent text-accent" : ""}`}
                        />
                      </button>
                    </div>
                  </td>
