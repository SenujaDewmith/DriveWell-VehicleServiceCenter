...
export function StaffDashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get(`/api/staff/my-services${params}`),
      api.get(`/api/staff/my-performance${params}`),
    ])
      .then(([svc, perf]) => {
                ...
                services.map((s) => (
                  <tr key={s.reservation_id} className="border-b border-border last:border-0">
                    ...
                    <td className="py-2 px-2 text-foreground">
                      {s.plate_no} — {s.make} {s.model}
                    </td>
                    <td className="py-2 px-2 text-foreground">{s.package_name}</td>
                    <td className="py-2 px-2 text-foreground">
                      {s.work_note ?? <span className="text-muted-foreground">—</span>}
                    </td>
