...
function aggregateToMonthly(byDate) {
  ...
  return MONTHS.filter((m) => totals[m] !== undefined).map((m) => ({
    month: m,
    revenue: totals[m],
  }));
}
...
export function ManagerOverview() {
  ...
  const [volumeByPackage, setVolumeByPackage] = useState([]);
  ...
  useEffect(() => {
    ...
    Promise.all([
      api.get(`/api/bookings?from=${today}&to=${today}`),
      api.get("/api/bookings"),
      api.get("/api/reports/revenue"),
      api.get("/api/reports/volume"),
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="border border-border rounded-lg bg-card p-4">
              <h3 className="text-sm font-semibold text-foreground mb-4">Monthly Revenue (LKR)</h3>
