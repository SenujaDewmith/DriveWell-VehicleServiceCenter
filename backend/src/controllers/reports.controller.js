...
const fetchRevenueSummary = async (from, to) => {
...
const getRevenueData = async (from, to) => {
  ...
  return { summary, by_package, by_date };
};

const getVolumeData = async (from, to) => {
  ...
  return { by_status, by_package, by_date };
};

const getStaffPerformanceData = async (from, to) => {
  const conditions = dateWhere(from, to);
...
const getActivityLogsData = async (from, to, limit = 100) => {
  const conditions = [];
...
const volumeReport = async (req, res) => {
  const { from, to } = req.query;
  try {
    const data = await getVolumeData(from, to);
    res.status(200).json(serialize(data));
  } catch (error) {
...
const staffPerformanceReport = async (req, res) => {
  const { from, to } = req.query;
  try {
    const data = await getStaffPerformanceData(from, to);
    res.status(200).json(serialize(data));
  } catch (error) {
...
const activityLog = async (req, res) => {
  ...
  try {
    const data = await getActivityLogsData(from, to, limit);
    res.status(200).json(serialize(data));
  } catch (error) {
...
const revenueReportPdf = async (req, res) => {
  const { from, to } = req.query;
  try {
    const { summary, by_package, by_date } = serialize(await getRevenueData(from, to));
    const prevRange = previousPeriod(from, to);
    const comparison = prevRange
      ? { prevSummary: serialize(await fetchRevenueSummary(prevRange.from, prevRange.to)), prevRange }
      : null;
    const doc = buildRevenuePdf({ summary, by_package, by_date, comparison, from, to, generatedBy: req.user?.email });
    sendPdf(res, doc, "drivewell-revenue-report.pdf");
  } catch (error) {
    logger.error(`revenueReportPdf failed — ${error.message}`);
    res.status(500).json({ message: "Server error" });
  }
};

const volumeReportPdf = async (req, res) => {
  const { from, to } = req.query;
  try {
    const { by_status, by_package, by_date } = serialize(await getVolumeData(from, to));
    const doc = buildVolumePdf({ by_status, by_package, by_date, from, to, generatedBy: req.user?.email });
    sendPdf(res, doc, "drivewell-volume-report.pdf");
  } catch (error) {
    logger.error(`volumeReportPdf failed — ${error.message}`);
    res.status(500).json({ message: "Server error" });
  }
};

const staffPerformanceReportPdf = async (req, res) => {
  const { from, to } = req.query;
  try {
    const { staff_performance } = serialize(await getStaffPerformanceData(from, to));
    const doc = buildStaffPerformancePdf({ staff_performance, from, to, generatedBy: req.user?.email });
    sendPdf(res, doc, "drivewell-staff-performance-report.pdf");
  } catch (error) {
    logger.error(`staffPerformanceReportPdf failed — ${error.message}`);
    res.status(500).json({ message: "Server error" });
  }
};

const activityLogPdf = async (req, res) => {
  const { from, to, limit = 200 } = req.query;
  try {
    const { logs } = serialize(await getActivityLogsData(from, to, limit));
    const doc = buildActivityPdf({ logs, from, to, generatedBy: req.user?.email });
    sendPdf(res, doc, "drivewell-activity-log.pdf");
  } catch (error) {
    logger.error(`activityLogPdf failed — ${error.message}`);
    res.status(500).json({ message: "Server error" });
  }
};
