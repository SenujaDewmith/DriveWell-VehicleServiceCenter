...
export function BookingCalendar({ packageId, selectedDate, onSelectDate }) {
    ...
    return (<div className="border rounded-lg overflow-hidden">
      ...
      <div className="p-2.5">
        {loading ? (<div className="flex justify-center py-10">
            ...
            <div className="grid grid-cols-7 gap-1 mb-1">
              {WEEKDAYS.map((w) => (<div key={w} className="text-center text-[10px] font-medium text-muted-foreground py-0.5">
                  {w}
                </div>))}
            </div>
            <div className="grid grid-cols-7 gap-1">
              {cells.map(({ date, inMonth }, i) => {
                ...
                return (<button key={i} type="button" disabled={disabled} onClick={() => onSelectDate(key)} className={`flex flex-col items-center justify-center gap-0.5 rounded-md py-1 text-xs transition-colors ${!inMonth || isPast
                        ? "text-muted-foreground/30 cursor-default"
                        : isSelected
                            ? "bg-cta text-cta-foreground font-semibold"
                            : isClosed
                                ? "text-muted-foreground/50 cursor-default"
                                : "hover:bg-muted cursor-pointer"} ${isToday && !isSelected ? "ring-1 ring-cta" : ""}`}>
                    <span>{date.getDate()}</span>
                    {inMonth && info && info.status !== "closed" && (<span className={`text-[9px] leading-none ${isSelected ? "text-cta-foreground/80" : "text-muted-foreground"}`}>
                        {info.remaining_capacity} slots
                      </span>)}
                    {inMonth && info && (<span className={`h-1 w-1 rounded-full ${STATUS_DOT[info.status]}`}/>)}
                  </button>);
            })}
            </div>
          </>)}
      ...
      <div className="flex flex-wrap items-center gap-3 px-3 py-2 border-t bg-muted/20">
        {LEGEND.map((l) => (<div key={l.status} className="flex items-center gap-1.5">
