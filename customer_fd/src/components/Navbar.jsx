      ...
      <div className="container mx-auto px-4">
        <div className="flex h-16 items-center justify-between">
          <Link to="/" className="flex items-center gap-2">
            <Logo className="h-9"/>
            <span className="text-xl font-bold text-foreground">DriveWell</span>
          </Link>
          ...
          <nav className="hidden md:flex items-center space-x-6">
            {isLoading ? (<>
                ...
                <Skeleton className="h-4 w-16"/>
              </>) : (navLinks.map((link) => (<Link key={link.to} to={link.to} className={`text-sm font-medium transition-colors hover:text-cta ${isActiveLink(link.to) ? "text-cta" : "text-foreground"}`}>
                  {link.label}
                </Link>)))}
          </nav>

          <div className="flex items-center space-x-4">
            <Button variant="ghost" size="icon" onClick={toggleTheme} className="text-foreground hover:text-cta">
              {theme === "light" ? <Moon className="h-5 w-5"/> : <Sun className="h-5 w-5"/>}
            </Button>

            {isLoading ? (<Skeleton className="h-8 w-8 rounded-full"/>) : user ? (<DropdownMenu>
        ...
        {mobileMenuOpen && (<div className="md:hidden py-4 border-t border-border">
            <nav className="flex flex-col space-y-3">
              {isLoading ? (<>
                  ...
                  {navLinks.map((link) => (<Link key={link.to} to={link.to} className={`text-sm font-medium transition-colors hover:text-cta ${isActiveLink(link.to) ? "text-cta" : "text-foreground"}`} onClick={() => setMobileMenuOpen(false)}>
                      {link.label}
                    </Link>))}
                  {!user && (<>
