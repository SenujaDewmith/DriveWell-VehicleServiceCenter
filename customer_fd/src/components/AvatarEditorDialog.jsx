...
export function AvatarEditorDialog({ imageSrc, onOpenChange, onSave }) {
    ...
    const handleDialogOpenChange = (open) => {
        if (isSaving)
            return; // don't allow dismissing mid-upload
        if (!open)
            onOpenChange(false);
    };
    ...
    return (<Dialog open={!!imageSrc} onOpenChange={handleDialogOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="relative h-72 w-full overflow-hidden rounded-md bg-muted">
          {imageSrc && isCropperReady ? (<Cropper image={imageSrc} crop={crop} zoom={zoom} aspect={1} cropShape="round" showGrid={false} onCropChange={setCrop} onZoomChange={setZoom} onCropComplete={(_, areaPixels) => setCroppedAreaPixels(areaPixels)}/>) : (<div className="flex h-full w-full items-center justify-center">
