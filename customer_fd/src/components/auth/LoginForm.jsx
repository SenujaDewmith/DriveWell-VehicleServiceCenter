...
export function LoginForm({ onSuccess, onSwitchToRegister }) {
    ...
    return (<div>
      ...
      {onSwitchToRegister && (<p className="mt-6 text-center text-sm text-muted-foreground">
          New to DriveWell?{" "}
          <button type="button" onClick={onSwitchToRegister} className="text-cta hover:underline font-medium">
            Create an account
          </button>
        </p>)}
    </div>);
