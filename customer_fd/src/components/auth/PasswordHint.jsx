...
export function PasswordHint({ password }) {
    if (!password) {
    ...
    if (!nextUnmet) {
        return (<p className="flex items-center gap-1.5 text-xs text-cta">
        <Check className="h-3 w-3 shrink-0"/>
        Strong password
      </p>);
    }
    return (<p className="flex items-center gap-1.5 text-xs text-muted-foreground">
