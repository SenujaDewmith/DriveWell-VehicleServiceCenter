...
export function AddVehicleDialog({ open, onOpenChange, onVehicleAdded }) {
        ...
        {loadingLookups ? (<div className="flex justify-center py-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2 col-span-2">
                ...
                <PlateNumberInput id="add-plate_no" value={plateNo} onChange={(next) => {
                setPlateNo(next);
                ...
                {formErrors.plate_no ? (<p className="text-xs text-destructive">{formErrors.plate_no}</p>) : (<p className="text-xs text-muted-foreground">Format: 2-3 letters followed by 4 digits (e.g. KA-1234 or CAB-1234)</p>)}
              </div>
            </div>
            <div className="flex justify-end gap-3 pt-2">
              ...
              <Button type="submit" className="bg-cta text-cta-foreground hover:bg-cta/90" disabled={saving}>
                {saving ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin"/>Adding...</>) : ("Add Vehicle")}
              </Button>
            </div>
          </form>)}
      </DialogContent>
