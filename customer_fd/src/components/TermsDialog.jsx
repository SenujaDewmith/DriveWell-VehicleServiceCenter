...
export function TermsDialog({ open, onOpenChange, onAgree }) {
      ...
      <DialogContent className="max-w-lg gap-0 p-0">
        ...
        <ScrollArea className="max-h-[min(55vh,24rem)]">
          <div className="space-y-5 p-6 text-sm">
            {TERMS_SECTIONS.map((section, idx) => (<section key={section.title}>
                <h4 className="mb-2 font-semibold text-foreground">
                  {idx + 1}. {section.title}
                </h4>
                <ul className="space-y-1.5">
                  {section.points.map((point) => (<li key={point} className="flex gap-2 text-muted-foreground">
        ...
        <DialogFooter className="gap-2 border-t p-4 sm:justify-between sm:gap-0">
          ...
          {onAgree && (<Button className="bg-cta text-cta-foreground hover:bg-cta/90" onClick={onAgree}>
