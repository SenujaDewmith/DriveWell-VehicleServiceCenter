...
export function VehicleStep({ vehicles, dataLoading, selectedVehicleId, onSelect, onAddVehicle, onBack, onContinue, }) {
    return (<Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        {dataLoading ? (<div className="flex justify-center py-8">
            <Loader2 className="h-6 w-6 animate-spin text-cta"/>
          </div>) : vehicles.length === 0 ? (<div className="text-center py-8">
        ...
        <ScrollFade className="grid gap-4 sm:grid-cols-2 max-h-[min(50vh,26rem)] overflow-y-auto overscroll-contain -m-1 p-1" deps={[vehicles.length]}>
            {vehicles.map((v) => (<Card key={v.vehicle_id} className={`cursor-pointer transition-all ${selectedVehicleId === v.vehicle_id ? "border-cta border-2 bg-cta/5" : "hover:border-cta/50"}`} onClick={() => onSelect(v.vehicle_id)}>
                <CardContent className="flex items-center gap-4 p-4">
                  ...
                  {selectedVehicleId === v.vehicle_id && (<CheckCircle className="h-6 w-6 text-cta"/>)}
                </CardContent>
              </Card>))}
            <Card className="cursor-pointer border-dashed hover:border-cta/50 transition-all" onClick={onAddVehicle}>
              <CardContent className="flex items-center gap-4 p-4">
        ...
        <div className="sticky bottom-0 -mx-6 -mb-6 rounded-b-lg border-t bg-card px-6 py-4 flex gap-3">
          ...
          <Button className="flex-1 bg-cta text-cta-foreground hover:bg-cta/90" onClick={onContinue} disabled={!selectedVehicleId}>
            Continue
          </Button>
        </div>
      </CardContent>
