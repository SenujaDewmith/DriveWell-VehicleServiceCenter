...
import { type AvailableSlot } from "@/services/bookings.service";
...
function renderStep(overrides: Partial<React.ComponentProps<typeof DateTimeStep>> = {}) {
  const onSelectSlot = vi.fn();
  const onContinue = vi.fn();
  const onBack = vi.fn();
  const onSelectDate = vi.fn();

  const props: React.ComponentProps<typeof DateTimeStep> = {
    packageId: 1,
    pkg: undefined,
