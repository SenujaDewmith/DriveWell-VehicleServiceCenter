...
export default function Profile() {
    ...
    const [removeAvatarError, setRemoveAvatarError] = useState(null);
    ...
    const handleAvatarEditorSave = async (blob) => {
    ...
    const handleConfirmRemoveAvatar = async () => {
        ...
        try {
            ...
            setRemoveAvatarError(error instanceof Error ? error.message : "Failed to remove avatar");
        }
        finally {
    ...
    return (<div className="container mx-auto px-4 py-6">
      <div className="mx-auto max-w-4xl space-y-4">
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-4 text-center sm:flex-row sm:text-left">
            <div className="relative shrink-0">
              ...
              {isSavingAvatar && (<div className="absolute inset-0 flex items-center justify-center rounded-full bg-background/60">
            ...
            <div className="flex shrink-0 gap-2">
              <input ref={avatarInputRef} type="file" accept={ALLOWED_AVATAR_TYPES.join(",")} className="hidden" onChange={handleAvatarFileSelected}/>
              <Button variant="outline" size="sm" onClick={handleChangeAvatarClick} disabled={isSavingAvatar}>
                Change Avatar
              </Button>
              {user.avatar && (<Button variant="ghost" size="sm" onClick={() => setIsRemoveDialogOpen(true)} disabled={isSavingAvatar}>
                  Remove
                </Button>)}
            </div>
          </CardContent>
        ...
        <AvatarEditorDialog imageSrc={editorImageSrc} onOpenChange={(open) => {
            if (!open) {
                if (editorImageSrc)
                    URL.revokeObjectURL(editorImageSrc);
        ...
        <AlertDialog open={isRemoveDialogOpen} onOpenChange={(open) => {
            if (isRemovingAvatar)
                return;
          ...
          <AlertDialogContent>
            ...
            {removeAvatarError && (<p className="text-sm text-destructive">{removeAvatarError}</p>)}
            <AlertDialogFooter>
              <Button variant="outline" onClick={() => setIsRemoveDialogOpen(false)} disabled={isRemovingAvatar}>
                Cancel
              </Button>
              <Button variant="destructive" onClick={handleConfirmRemoveAvatar} disabled={isRemovingAvatar}>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Card className="flex flex-col">
            ...
            <CardContent className="flex flex-1 flex-col">
              <form onSubmit={handlePasswordSubmit(onChangePassword)} className="flex flex-1 flex-col space-y-3">
                <div className="space-y-1.5">
                  ...
                  {passwordErrors.currentPassword && (<p className="text-sm text-destructive">
            ...
            : "Show password"} tabIndex={-1}>
                  ...
                  {passwordErrors.confirmNewPassword && (<p className="text-sm text-destructive">
