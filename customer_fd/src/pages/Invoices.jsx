...
export default function Invoices() {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      {!loading && !error && invoices.length === 0 && (<Card className="text-center py-12">
          <CardContent>
      ...
      {!loading && !error && invoices.length > 0 && (<div className="space-y-4">
          {invoices.map((invoice) => (<Card key={invoice.invoice_id} className="hover:shadow-lg transition-shadow">
                ...
                <div className="flex flex-col lg:flex-row gap-6">
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    : "bg-status-booked/10 text-status-booked border-status-booked/20"}>
                      ...
                      {invoice.has_oil_change && (<div className="mt-4 flex items-center gap-3 rounded-lg border-2 border-cta/20 bg-cta/5 px-4 py-3">
                      ...
                      <div className="mt-4 pt-4 border-t border-border space-y-2">
                        ...
                        {invoice.items.map((item) => (<div key={item.invoice_item_id} className="flex justify-between text-sm">
                            <span className="text-muted-foreground">
                              {item.description}{item.quantity > 1 ? ` ×${item.quantity}` : ""}
                            </span>
                            <span className="font-medium">LKR {parseFloat(item.line_total).toLocaleString()}</span>
                          </div>))}
                        {parseFloat(invoice.discount) > 0 && (<div className="flex justify-between text-sm">
