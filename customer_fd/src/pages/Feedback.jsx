...
const FEEDBACK_ELIGIBLE_STATUSES = ["Collected"];
function fmtDate(d) {
...
export default function Feedback() {
    ...
    useEffect(() => {
        ...
        let cancelled = false;
        setLoading(true);
        Promise.all([bookingsService.getBookings(), feedbackService.getFeedback()])
            .then(([bookingsRes, feedbackRes]) => {
            if (cancelled)
                return;
    ...
    const eligibleBookings = bookings.filter((b) => FEEDBACK_ELIGIBLE_STATUSES.includes(b.status) && !reviewedReservationIds.has(b.reservation_id));
    const effectiveSelectedId = selectedBookingId && eligibleBookings.some((b) => b.reservation_id.toString() === selectedBookingId)
        ? selectedBookingId
        : (eligibleBookings[0]?.reservation_id.toString() ?? "");
    const averageRating = feedback.length > 0 ? (feedback.reduce((sum, f) => sum + f.rating, 0) / feedback.length).toFixed(1) : "N/A";
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {loading ? (<div className="flex justify-center items-center py-24">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="space-y-2">
                    ...
                    <Select value={effectiveSelectedId} onValueChange={setSelectedBookingId}>
                      ...
                      <SelectContent>
                        {eligibleBookings.map((b) => (<SelectItem key={b.reservation_id} value={b.reservation_id.toString()}>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[1, 2, 3, 4, 5].map((r) => (<button key={r} type="button" onClick={() => setRating(r)} className="group" aria-label={`${r} star${r !== 1 ? "s" : ""}`} aria-pressed={r <= rating}>
                          <Star className={`h-10 w-10 transition-colors ${r <= rating ? "fill-cta text-cta" : "text-muted-foreground group-hover:text-cta"}`}/>
                        </button>))}
                    </div>
                  </div>
                  <div>
                    ...
                    <Textarea id="comment" placeholder="Tell us about your experience..." rows={4} value={comment} onChange={(e) => setComment(e.target.value)}/>
                  </div>
                  <Button type="submit" className="w-full bg-cta text-cta-foreground hover:bg-cta/90" disabled={submitting}>
                    {submitting ? (<>
          ...
          <Card>
            ...
            <CardContent>
              {feedback.length === 0 ? (<div className="text-center py-8">
                  <p className="text-muted-foreground">No feedback submitted yet</p>
                </div>) : (<div className="space-y-6">
                  {feedback.map((f) => (<div key={f.feedback_id} className="border-b border-border last:border-0 pb-6 last:pb-0">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <div className="flex gap-1">
                          {Array.from({ length: f.rating }).map((_, i) => (<Star key={i} className="h-5 w-5 fill-cta text-cta"/>))}
                        </div>
                      </div>
