...
export default function Services() {
    ...
    return (<div className="container mx-auto px-4 py-8">
      ...
      {!loading && error && (<Card className="text-center py-12 border-destructive/30">
          <CardContent>
            <p className="text-destructive">{error}</p>
          </CardContent>
        </Card>)}

      {!loading && !error && packages.length === 0 && (<Card className="text-center py-12">
          <CardContent>
            <p className="text-muted-foreground">No service packages are currently available.</p>
          </CardContent>
        </Card>)}

      {!loading && !error && packages.length > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {packages.map((pkg) => {
                const bullets = getBullets(pkg.description);
                const src = imageSrc(pkg.image_url);
                return (<Card key={pkg.package_id} className="flex flex-col hover:shadow-xl transition-all overflow-hidden pt-0">
                <div className="aspect-video w-full bg-muted flex items-center justify-center overflow-hidden relative">
                  {pkg.package_code && (<span className="absolute top-2 left-2 z-10 rounded-md bg-background/90 px-2 py-0.5 text-xs font-mono font-medium text-foreground shadow-sm">
                      {pkg.package_code}
                    </span>)}
                  {src ? (<img src={src} alt={pkg.name} className="h-full w-full object-cover"/>) : (<Car className="h-10 w-10 text-muted-foreground"/>)}
                </div>
                ...
                <CardContent className="flex-1 flex flex-col p-4 pt-0">
                  {bullets.length > 0 ? (<ul className="space-y-1.5 flex-1 mb-4">
                      {bullets.map((item, i) => (<li key={i} className="flex items-start gap-2">
                          <CheckCircle className="h-4 w-4 text-cta shrink-0 mt-0.5"/>
                          <span className="text-xs">{item}</span>
                        </li>))}
                    </ul>) : (<div className="flex-1 mb-4"/>)}
                  <Button size="sm" className="w-full bg-cta text-cta-foreground hover:bg-cta/90" onClick={() => navigate(`/book?package=${pkg.package_id}`)}>
                    Book This Service
                  </Button>
                </CardContent>
              </Card>);
